...
#define EVENT_SET_AND_EXEC_CB(btn, evt)                                        \
    ...
    {                                                                          \
        ...
        bx_public( id, (USM_BTN_FIRST + evt), btn->id, 0);                      \
    } while(0)
#endif
/**
 * BTN_IS_PRESSED
...
void EVENT_SET_AND_EXEC_CB(flex_button_t* btn,uint16_t evt)
{
    s32 id = user_service_button_id();
	btn->event = evt;  
...
static void flex_button_read(void)
{
	...
	//bxsh_logln( "g_btn_status_reg:0x%X , g_logic_level:0x%X  ,raw_data:0x%x", g_btn_status_reg, g_logic_level ,raw_data);
}
