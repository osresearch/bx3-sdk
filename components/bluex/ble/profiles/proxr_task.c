 ...
 */
 #include "shell_api.h"
// #include "hf_service_app.h"
// #include "hf_service_ble.h"
// #include "bx_kernel.h"
                                      ...
                                      ke_task_id_t const src_id)
{
    ...
    bxs_logln("proxr gattc_write_req_ind_handler");
    if(ke_state_get(dest_id) == PROXR_IDLE)
    {
        ...
        if(param->value[0] <= PROXR_ALERT_HIGH)
        {
            ...
            bxs_logln("alert_lvl:%d",alert_lvl);
...
static int gattc_read_req_ind_handler(ke_msg_id_t const msgid,
                                      ...
                                      ke_task_id_t const src_id)
{
    bxs_logln("proxr gattc_read_req_ind_handler");
    int msg_status = KE_MSG_CONSUMED;
...
static int gapm_dev_adv_tx_power_ind_handler(ke_msg_id_t const msgid,
                                            ...
                                            ke_task_id_t const src_id)
{
    bxs_logln("proxr gapm_dev_adv_tx_power_ind_handler");
    // Get the address of the environment
    struct proxr_env_tag *proxr_env = PRF_ENV_GET(PROXR, proxr);
...
static int gapm_cmp_evt_handler(ke_msg_id_t const msgid,
                                ...
                                ke_task_id_t const src_id)
{
    bxs_logln("proxr gapm_cmp_evt_handler");
    // Get the address of the environment
    struct proxr_env_tag *proxr_env = PRF_ENV_GET(PROXR, proxr);
