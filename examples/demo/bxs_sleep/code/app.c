...
void app_init( void )
{
    ...
    bx_call( bxs_gpio_a_id(), BXM_WRITE, 2, 0 );
    bx_defer( user_service_id, BXM_SLEEP_UNLOCK, 0, 0, 3000 );
}
...
bx_err_t user_msg_handle_func( s32 id, u32 msg, u32 param0, u32 param1 )
{
    ...
    switch( msg ) {
        case BXM_SLEEP_UNLOCK: {
            bx_defer( user_service_id, BXM_SLEEP_LOCK, 0, 0, 3000 );
            bx_call( bxs_gpio_a_id(), BXM_CLOSE, 0, 0 );
        }
        break;
        case BXM_SLEEP_LOCK: {
            bx_call( bxs_gpio_a_id(), BXM_OPEN, 0, 0 );
