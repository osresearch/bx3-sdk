...
void parse_tp_raw_data(u8 * p_data)
{
    //SHOW_ARRAY(p_data,7);
    
    tp_data.guesture = p_data[1];
    tp_data.fg_num = p_data[2];
    tp_data.state = p_data[3]>>4;
    tp_data.pos_x = (u16)((p_data[3]&0x0f)<<8)+(u16)(p_data[4]);
    tp_data.pos_y = (u16)((p_data[5]&0x0f)<<8)+(u16)(p_data[6]);
    
    
    LOG_V("guesture :%x",tp_data.guesture);
    LOG_V("fg_num   :%u",tp_data.fg_num);
    LOG_V("state    :%u",tp_data.state);
    LOG_V("pos_x    :%u",tp_data.pos_x);
    LOG_V("pos_y    :%u",tp_data.pos_y);
}
/** ---------------------------------------------------------------------------
 * @brief   :
 * @note    :
    ...
    switch( msg ) {
        ...
        case USM_TP_ACT_GET_ID: {
            ...
            if( tp_svc.state == tp_sta_busy ) {
                ...
                tp_svc.state = tp_sta_busy;
                u8 whoami = 0;
                bx_err_t err = bxd_iic_read_ex( BX_IIC0, 0x2a, 0xa7, &whoami, 1, 100 );
                tp_svc.state = tp_sta_ready;
                if( err == BX_OK ) {
        ...
        case USM_TP_ACT_UPDATE_GUESTURE: {
            ...
            if( tp_svc.state == tp_sta_busy ) {
                ...
                u8 guestureid = 0;
                tp_svc.state = tp_sta_busy;
                bx_err_t err = bxd_iic_read_ex( BX_IIC0, 0x2a, 0x01, &guestureid, 1, 100 );
                tp_svc.state = tp_sta_ready;
                if( err == BX_OK ) {
        ...
        case USM_TP_ACT_UPDATA_ALL_DATA: {
            ...
            if( tp_svc.state == tp_sta_busy ) {
                ...
                u8 tp_raw_data[10]={0};
                tp_svc.state = tp_sta_busy;
                bx_err_t err = bxd_iic_read_ex( BX_IIC0, 0x2a, 0x00,tp_raw_data, 7, 200 );
                tp_svc.state = tp_sta_ready;
                if( err == BX_OK ) {
                    parse_tp_raw_data(tp_raw_data);
...
//            bx_post( tp_svc.id, USM_TP_ACT_UPDATE_POSITION, 0, 0 );
            bx_post( tp_svc.id, USM_TP_ACT_UPDATA_ALL_DATA, 0, 0 );
            return BX_OK;
        }
...
void us_tp_init( void )
{
    ...
    bxd_iic_set_scl( BX_IIC0, TP_SCL_PIN );
    bxd_iic_set_sda( BX_IIC0, TP_SDA_PIN );
