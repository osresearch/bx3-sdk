...
//    bxsh_logln("user gattc_write_req_ind_handler");
    ...
    if( state == USER_PROFILE_IDLE ) {
        ...
        static u8 rec_buf[20];
        memcpy( rec_buf, param->value, param->length );

        if( rec_buf[0] == 0xff && rec_buf[6] == 0xef ) {
            static u32 freq;
            static u8 duty;
            static u32 freq_buf[4];
            freq_buf[0] = rec_buf[1];
            freq_buf[1] = rec_buf[2];
            freq_buf[2] = rec_buf[3];
            freq_buf[3] = rec_buf[4];

            freq = ( freq_buf[0] << 24 ) + ( freq_buf[1] << 16 ) + ( freq_buf[2] << 8 ) + freq_buf[3];
            duty = rec_buf[5];
            
            bx_call( bxs_pwm0_id(), BXM_START, freq, duty );  //Open the PWM
