...
/* private variables ---------------------------------------------------------*/
static struct bxs_gpio gpioa_svc = { 0 };
...
{
    struct bxs_gpio * p_svc;
    GET_GPIO_SERVICE_BY_ID( p_svc, id );

    switch( msg ) {
        case BXM_OPEN : {
            bx_pm_lock( BX_PM_GPIO );
            return bxd_gpio_open( p_svc->handle );
        }

        case BXM_CLOSE : {
            bx_pm_unlock( BX_PM_GPIO );
            return bxd_gpio_close( p_svc->handle );
        }

        case BXM_READ :
            return bxd_gpio_read( p_svc->handle, ( u32 * )param0 );

        case BXM_WRITE :
            return bxd_gpio_write( p_svc->handle, param0, param1 );

        case BXM_TOGGLE:
            return bxd_gpio_toggle( p_svc->handle, param0 );
...
static bx_err_t gpio_property_set( s32 id, u32 property, u32 param0, u32 param1 )
{
    ...
    switch( property ) {
        case BXP_MODE:
            bxd_gpio_set_mode( p_svc->handle, param0, param1 );
            break;

        case BXP_GPIO_PULL:
            bxd_gpio_set_pull( p_svc->handle, param0, param1 );
            break;
...
{
    memset( &gpioa_svc, 0, sizeof( struct bxs_gpio ) );
