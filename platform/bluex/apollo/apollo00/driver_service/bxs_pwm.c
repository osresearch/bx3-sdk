...
/* private variables ---------------------------------------------------------*/
static struct bxs_pwm pwm0_svc = { 0 };
static struct bxs_pwm pwm1_svc = { 0 };
static struct bxs_pwm pwm2_svc = { 0 };
static struct bxs_pwm pwm3_svc = { 0 };
static struct bxs_pwm pwm4_svc = { 0 };
...
{
    struct bxs_pwm * p_svc;
    GET_PWM_SERVICE_BY_ID( p_svc, id );

    switch( msg ) {
        case BXM_OPEN : {
            bx_pm_lock( BX_PM_PWM );
            return bxd_pwm_open( p_svc->handle );
        }

        case BXM_CLOSE : {
            bx_pm_unlock( BX_PM_PWM );
            return bxd_pwm_close( p_svc->handle );
        }

        case BXM_START :
            bxd_pwm_start( p_svc->handle, param0, param1 );
            break;

        case BXM_STOP :
            bxd_pwm_stop( p_svc->handle );
            break;
...
static bx_err_t pwm_property_set( s32 id, u32 property, u32 param0, u32 param1 )
{
    ...
    switch( property ) {
        case BXP_PIN:
            err = bxd_pwm_set_pin( p_svc->handle, param0 );
            if( err == BX_OK ) {
