...
/* private variables ---------------------------------------------------------*/
static struct bxs_timer tim0_svc = { 0 };
static struct bxs_timer tim1_svc = { 0 };
...
{
    struct bxs_timer * p_svc;
    GET_TIM_SERVICE_BY_ID( p_svc, id );

    switch( msg ) {
        case BXM_OPEN : {
            bx_pm_lock( BX_PM_TIM );
            return bxd_timer_open( p_svc->handle );
        }

        case BXM_CLOSE : {
            bx_pm_unlock( BX_PM_TIM );
            return bxd_timer_close( p_svc->handle );
        }

        case BXM_START :
            bxd_timer_start( p_svc->handle, param0 );
            break;
