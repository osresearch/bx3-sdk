...
/* private variables ---------------------------------------------------------*/
static struct bxs_spim spim0_svc = { 0 };
static struct bxs_spim spim1_svc = { 0 };
...
static bx_err_t spim_msg_handle( s32 id, u32 msg, u32 param0, u32 param1 )
{
    ...
    switch( msg ) {
        case BXM_OPEN: {
            ...
            return bxd_spim_open( p_svc->handle );
        }

        case BXM_CLOSE: {
            bx_pm_unlock( BX_PM_SPIM );
            return bxd_spim_close( p_svc->handle );
        }

        case BXM_READ:
            return bxd_spim_read( p_svc->handle, ( u8 * )param0, param1 );

        case BXM_WRITE:
            return bxd_spim_write( p_svc->handle, ( u8 * )param0, param1 );

        case BXM_SPIM_TXRX_START:
            bxd_spim_transmit_recieve(   p_svc->handle,  \
                                         p_cfg->tx_buff, \
                                         p_cfg->tx_len,  \
                                         p_cfg->rx_buff, \
                                         p_cfg->rx_len );
...
static bx_err_t spim_property_set( s32 id, u32 property, u32 param0, u32 param1 )
{
    ...
    switch( property ) {
        case BXP_SPIM_CS1_PIN:
            return bxd_spim_set_cs1_pin( p_svc->handle, param0 );

        case BXP_SPIM_USE_CS_NUM:
            return bxd_spim_use_cs( p_svc->handle, param0 );

        case BXP_DATA_BIT:
            return bxd_spim_set_data_bit( p_svc->handle, param0 );

        case BXP_SPEED:
            return bxd_spim_set_speed( p_svc->handle, param0 );
