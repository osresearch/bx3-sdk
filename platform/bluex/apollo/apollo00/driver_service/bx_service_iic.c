...
/* private variables ---------------------------------------------------------*/
static struct bx_iic_service iic0_svc = { 0 };
static struct bx_iic_service iic1_svc = { 0 };
/* exported variables --------------------------------------------------------*/
 ...
 * @retval  :
-----------------------------------------------------------------------------*/
static bx_err_t iic_svc_read_action( struct bx_iic_service * p_svc, u8 * buff, u32 len )
{
    if( p_svc->lock ) {
        return BX_ERR_BUSY;
    }
    p_svc->lock = true;
    bx_err_t err = bx_drv_iic_read( p_svc->handle, p_svc->addr, p_svc->reg, buff, len );
    p_svc->lock = false;
 ...
 * @retval  :
-----------------------------------------------------------------------------*/
static bx_err_t iic_svc_write_action( struct bx_iic_service * p_svc, u8 * buff, u32 len )
{
    if( p_svc->lock ) {
        return BX_ERR_BUSY;
    }
    ...
    p_svc->lock = true;
    bx_err_t err = bx_drv_iic_write( p_svc->handle, p_svc->addr, p_svc->reg, buff, len );
    p_svc->lock = false;
...
static bx_err_t iic_svc_property_set( s32 id, u32 property, u32 param0, u32 param1 )
{
    ...
    switch( property ) {
        ...
        case BXP_MODE:
            return bx_drv_iic_set_mode( p_svc->handle, param0 );

        case BXP_SPEED:
            return bx_drv_iic_set_speed( p_svc->handle, param0 );

        case BXP_ADDR_BIT:
            return bx_drv_iic_set_addr_bit( p_svc->handle, param0 );
        ...
        case BXP_IIC_PIN_SDA:
            return bx_drv_iic_set_sda( p_svc->handle, param0 );

        case BXP_IIC_PIN_SCL:
            return bx_drv_iic_set_scl( p_svc->handle, param0 );
